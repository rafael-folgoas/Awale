...
void enregistrerCoupHistorique(Jeu *jeu, Client *joueur, int coup) {
    ...
    sprintf(buffer + strlen(buffer), "Score %s: %d | %s: %d\n", jeu->joueur1->name, jeu->scoreJ1, jeu->joueur2->name, jeu->scoreJ2);
    if(jeu->joueur1->sauvegardeMode){
        //FILE J1 
        char historiquePath1[strlen("data/") + strlen(jeu->joueur1->name) + strlen("/historiquesParties.txt") + 1];
        sprintf(historiquePath1, "data/%s/historiquesParties.txt", jeu->joueur1->name);
    ...
    if(jeu->joueur2->sauvegardeMode){
        //FILE J2 
        char historiquePath2[strlen("data/") + strlen(jeu->joueur2->name) + strlen("/historiquesParties.txt") + 1];
        sprintf(historiquePath2, "data/%s/historiquesParties.txt", jeu->joueur2->name);
...
void ecrireDebutPartieHistoJoueur(Jeu *jeu){
    ...
    if (jeu->joueur1->sauvegardeMode){
        char historiquePath[strlen("data/") + strlen(jeu->joueur1->name) + strlen("/historiquesParties.txt") + 1];
        sprintf(historiquePath, "data/%s/historiquesParties.txt", jeu->joueur1->name);
        ...
        fprintf(historiquesFile, "\n+ %s VS %s --------------", jeu->joueur1->name, jeu->joueur2->name);
    ...
    if (jeu->joueur2->sauvegardeMode){
        ...
        fprintf(historiquesFile2, "\n+ %s VS %s --------------", jeu->joueur2->name, jeu->joueur1->name);
...
void ecrireFinPartieHistoJoueur(Jeu *jeu){
    char* gagnant;
    if (jeu->gagnant==0){
        gagnant=jeu->joueur1->name;
    }else{
        gagnant=jeu->joueur2->name;
    }
    if (jeu->joueur1->sauvegardeMode){
        // Création du chemin du fichier historiquesPartie.txt
        char historiquePath[strlen("data/") + strlen(jeu->joueur1->name) + strlen("/historiquesParties.txt") + 1];
